...
import com.tinz.ntyw.dao.WaterReportMinuteDao;
import com.tinz.ntyw.entity.Factor;

@Service
public class WaterReportMinuteServiceImpl implements WaterReportMinuteService {

	private Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private WaterReportMinuteDao mDao;
	@Autowired
	...
	public List<Map<String, Object>> queryByType(Integer entpId, Integer portId, Date startTime, Date endTime) {
		...
		if(startTime == null) {
			Date now = new Date();
			startTime = new Date(now.getTime()-3*60*60*1000l);
		}
		if(endTime == null) {
			Date now = new Date();
			endTime = now;
