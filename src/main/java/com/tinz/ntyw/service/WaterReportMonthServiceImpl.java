...
import com.tinz.ntyw.dao.WaterReportMonthDao;
import com.tinz.ntyw.entity.Factor;

@Service
public class WaterReportMonthServiceImpl implements WaterReportMonthService {

	private Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private WaterReportMonthDao mDao;
	@Autowired
	...
	public List<Map<String, Object>> queryByType(Integer entpId, Integer portId, Date startTime, Date endTime) {
		...
		if(startTime == null) {
			Date now = new Date();
			startTime = new Date(now.getTime()-366*24*60*60*1000l);
		}
		if(endTime == null) {
			Date now = new Date();
			endTime = now;
