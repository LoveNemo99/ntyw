...
import com.tinz.ntyw.dao.WaterReportYearDao;
import com.tinz.ntyw.entity.Factor;

@Service
public class WaterReportYearServiceImpl implements WaterReportYearService {

	private Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private WaterReportYearDao mDao;
	@Autowired
	...
	public List<Map<String, Object>> queryByType(Integer entpId, Integer portId, Date startTime, Date endTime) {
		...
		if(startTime == null) {
			Date now = new Date();
			startTime = new Date(now.getTime()-10*365*24*60*60*1000l);
		}
		if(endTime == null) {
			Date now = new Date();
			endTime = now;
