...
import com.tinz.ntyw.dao.WaterReportHourDao;
import com.tinz.ntyw.entity.Factor;

@Service
public class WaterReportHourServiceImpl implements WaterReportHourService {

	private Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private WaterReportHourDao mDao;
	@Autowired
	...
	public List<Map<String, Object>> queryByType(Integer entpId, Integer portId, Date startTime, Date endTime) {
		...
		if(startTime == null) {
			Date now = new Date();
			startTime = new Date(now.getTime()-24*60*60*1000l);
		}
		if(endTime == null) {
			Date now = new Date();
			endTime = now;
		...
		List<Factor> factors = factorDao.queryByTypeId(2);//污水
